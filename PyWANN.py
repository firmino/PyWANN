...
class Discriminator:
        ...
        def __init__(self, retina_length, num_bits_addr = 2, memories_values_cummulative = False, randomize_positions = True):
                ...
                if (retina_length % num_bits_addr) > 0:
                        raise Exception('imcompatible number of memories for size of retina. Change mumber of addr bits')
        ...
        def training(self, list_positive_retina):
                for retina in list_positive_retina: 
                        for memory_key in self.__memories_mapping:
                                addr_list = []
                                position_list = self.__memories_mapping[memory_key]

                                for position in position_list:
                                        if retina[position] > 0:
                                                addr_list.append(1)
                                        else:
                                                addr_list.append(0)
                                self.__memories[memory_key].add_value(addr_list)
